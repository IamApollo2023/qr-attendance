...
export default function NonJILRelatedPage() {
  const { success, error: showError } = useToastContext();
  const { albums, loading, createAlbum, updateAlbum, removeAlbum } =
    useActivityAlbums({
      activityType: "non-jil-related",
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      <div className="mb-6">
        <h1 className="text-lg md:text-3xl font-semibold tracking-tight mb-2">
          Non JIL related activities
        </h1>
        <p className="text-xs md:text-base text-muted-foreground">
          Manage Non JIL related activity albums
        </p>
      </div>
