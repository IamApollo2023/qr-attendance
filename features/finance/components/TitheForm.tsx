...
interface TitheFormProps {
  tithe: Tithe | null;
  onSubmit: (data: CreateTitheInput | UpdateTitheInput) => Promise<void>;
  onCancel: () => void;
}

export function TitheForm({ tithe, onSubmit, onCancel }: TitheFormProps) {
  const [formData, setFormData] = useState({
    amount: tithe?.amount || "",
    date: tithe?.date || new Date().toISOString().split("T")[0],
    notes: tithe?.notes || "",
  });
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    // Reset form when tithe changes
    setFormData({
      amount: tithe?.amount || "",
      date: tithe?.date || new Date().toISOString().split("T")[0],
      notes: tithe?.notes || "",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 pt-3 sm:pt-4 gap-2 sm:gap-0">
        ...
        >
          {loading ? "Saving..." : tithe ? "Update" : "Create"}
        </button>
      </div>
    </form>
  );
